...
public class LineChartView extends ChartView {
	...
	public void onDrawChart(Canvas canvas, ArrayList<ChartSet> data) {
		...
		for (ChartSet set : data) {
			...
			if (lineSet.isVisible()) {
				...
				if (lineSet.isDashed())
					mStyle.mLinePaint.setPathEffect(new DashPathEffect(
							new float[] { 10, 10 }, lineSet.getPhase()));
				else
	...
	private void drawPoints(Canvas canvas, LineSet set) {
		...
		int end = set.getEnd();
		int[] kkk = set.getIsShowPoint();
		for (int i = begin; i < end; i++) {
			for (int j = 0; j < kkk.length; j++) {
				if (i == kkk[j]) {
					path.addCircle(set.getEntry(i).getX(), set.getEntry(i)
							.getY(), set.getDotsRadius(), Path.Direction.CW);
					if (dotsBitmap != null)
						canvas.drawBitmap(dotsBitmap, set.getEntry(i).getX()
								- dotsBitmapWidthCenter, set.getEntry(i).getY()
								- dotsBitmapHeightCenter, mStyle.mDotsPaint);
				}
			}
		}
	...
	private void drawSmoothLine(Canvas canvas, LineSet set) {
		...
		Path path = new Path();
		path.moveTo(set.getEntry(set.getBegin()).getX(),
				set.getEntry(set.getBegin()).getY());

		Path bgPath = new Path();
		bgPath.moveTo(set.getEntry(set.getBegin()).getX(),
				set.getEntry(set.getBegin()).getY());
	...
	private void drawBackground(Canvas canvas, Path path, LineSet set,
		...
		if (set.hasGradientFill())
			mStyle.mFillPaint.setShader(new LinearGradient(super
					...
					.getInnerChartLeft(), innerChartBottom, set
