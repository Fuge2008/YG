...
public abstract class ChartView extends RelativeLayout
{
	...
	public ChartView(Context context, AttributeSet attrs)
	{
		...
		horController = new XController(this, context.getTheme().obtainStyledAttributes(attrs, R.styleable.ChartAttrs, 0, 0));
		verController = new YController(this, context.getTheme().obtainStyledAttributes(attrs, R.styleable.ChartAttrs, 0, 0));
	...
	private void digestData()
	{
		...
		for (ChartSet set : data)
		{
			...
			{
				set.getEntry(i).setCoordinates(horController.parsePos(i, set.getValue(i)), verController.parsePos(i, set.getValue(i)));
			}
		}
	}
	 ...
	 * @param data
	 *            {@link java.util.ArrayList} of
	 *            {@link com.db.chart.model.ChartSet} to use while drawing the
	 *            Chart
	 */
	 ...
	 *
	 * @param setIndex
	 *            Dataset's index to be displayed
	 */
	 ...
	 *
	 * @param anim
	 *            Animation used while showing and updating sets
	 */
	 ...
	 *
	 * @param setIndex
	 *            Dataset's index to be dismissed
	 */
	 ...
	 * @param bool
	 *            False if the tooltip should not be forced to be inside
	 ...
	 *
	 * @param index
	 *            Position of {@link ChartSet}
	 * @param anim
	 *            Animation extending {@link BaseStyleAnimation}
	 */
	...
	private void drawThresholdLine(Canvas canvas)
	{
		...
		canvas.drawLine(getInnerChartLeft(), mThresholdValue, getInnerChartRight(), mThresholdValue, style.thresholdPaint);
	}
	...
	private void drawVerticalGrid(Canvas canvas)
	{
		...
		{
			canvas.drawLine(pos, getInnerChartBottom(), pos, getInnerChartTop(), style.gridPaint);
		}
		...
		if (horController.borderSpacing != 0 || horController.mandatoryBorderSpacing != 0)
		{
			if (verController.labelsPositioning == YController.LabelPosition.NONE)
				canvas.drawLine(getInnerChartLeft(), getInnerChartBottom(), getInnerChartLeft(), getInnerChartTop(), style.gridPaint);
			canvas.drawLine(getInnerChartRight(), getInnerChartBottom(), getInnerChartRight(), getInnerChartTop(), style.gridPaint);
		}
	}
	...
	private void drawHorizontalGrid(Canvas canvas)
	{
		...
		{
			canvas.drawLine(getInnerChartLeft(), pos, getInnerChartRight(), pos, style.gridPaint);
		}
				...
				if (mEntryListener != null && mSetClicked != -1 && mIndexClicked != -1)
				{
					if (mRegions.get(mSetClicked).get(mIndexClicked).contains((int) event.getX(), (int) event.getY()))
					{
						...
						mEntryListener.onClick(mSetClicked, mIndexClicked,
								new Rect(mRegions.get(mSetClicked).get(mIndexClicked).getBounds().left - getPaddingLeft(), mRegions.get(mSetClicked)
										.get(mIndexClicked).getBounds().top
										- getPaddingTop(), mRegions.get(mSetClicked).get(mIndexClicked).getBounds().right - getPaddingLeft(),
										mRegions.get(mSetClicked).get(mIndexClicked).getBounds().bottom - getPaddingTop()), ((Integer) getTag()));
					}
					mSetClicked = -1;
					mIndexClicked = -1;
	 ...
	 *
	 * @param position
	 *            NONE - No labels OUTSIDE - Labels will be positioned outside
	 *            the chart INSIDE - Labels will be positioned inside the chart
	 */
	 ...
	 *
	 * @param type
	 *            {@link GridType} for grid
	 * @param paint
	 *            The Paint instance that will be used to draw the grid If null
	 *            the grid won't be drawn
	 */
	...
	/**
	 * Mandatory horizontal border when necessary (ex: BarCharts) Sets the
	 * attribute depending on the chart's orientation. e.g. If orientation is
	 * VERTICAL it means that this attribute must be handled by horizontal axis
	 * and not the vertical axis.
	 */
	...
	class Style
	{
		...
		protected Style(TypedArray attrs)
		{
			...
			axisThickness = attrs.getDimension(R.styleable.ChartAttrs_chart_axisThickness, getResources().getDimension(R.dimen.axis_thickness));

			labelColor = attrs.getColor(R.styleable.ChartAttrs_chart_labelColor, DEFAULT_COLOR);
			fontSize = attrs.getDimension(R.styleable.ChartAttrs_chart_fontSize, getResources().getDimension(R.dimen.font_size));
