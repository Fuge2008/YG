...
/**
 * Data model containing a set of {@link Point} to be used by
 * {@link com.db.chart.view.LineChartView}.
 */
public class LineSet extends ChartSet {
	 ...
	 * 设置哪几个点需要画圆
	 * 
	 * @see 注意：该方法必须在addPoints方法后面调用。否则无效
	 * 
	 * @param isShowPoint
	 */
	public void setIsShowPoint(int[] isShowPoint) {
		this.isShowPoint = isShowPoint;
	}

	public void addPoints(String[] labels, float[] values) {
		isShowPoint = new int[values.length];
		for (int i = 0; i < values.length; i++) {
			isShowPoint[i] = i;
		}
		if (labels.length != values.length)
			Log.e(TAG, "Arrays size doesn't match.",
	 ...
	 *
	 * @param thickness
	 *            Line thickness. Can't be equal or less than 0
	 */
	 ...
	 *
	 * @param thickness
	 *            Grid thickness. Can't be equal or less than 0
	 */
	 ...
	 *
	 * @param colors
	 *            The colors to be distributed among gradient
	 * @param positions
	 * @return
	 */
	 ...
	 *
	 * @param index
	 *            Index where the set begins
	 */
	public LineSet beginAt(int index) {

		if (index < 0)
			Log.e(TAG, "Index can't be negative.",
					new IllegalArgumentException());
		mBegin = index;
		return this;
	}
	...
	public LineSet endAt(int index) {
		...
		if (index > size())
			Log.e(TAG, "Index cannot be greater than the set's size.",
					new IllegalArgumentException());
		mEnd = index;
		return this;
	}
