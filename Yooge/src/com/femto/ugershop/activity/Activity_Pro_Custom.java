...
public class Activity_Pro_Custom extends BaseActivity {
	...
	private TextView tv_name_ocd, tv_time_ocd, tv_price_ocd, tv_nub_ocd, tv_count_ocd, tv_math_ocd, tv_status_ocd, tv_sure_ocd,
			tv_title_oderdetails, tv_faqi, tv_ordercode, tv_fist, tv_suershouh, tv_applytuik, tv_all;
	private EditText tv_ordercode_ocd;
	private LinearLayout ll_order_pro_custom;
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 1:
				...
				listThree.clear();
				listTwo.clear();
				Toast.makeText(Activity_Pro_Custom.this, "提交成功", Toast.LENGTH_SHORT).show();
				getData();
				break;
	...
	public void onClick(View v) {
		...
		case R.id.tv_faqi:
			...
			break;
		case R.id.tv_applytuik:
			showuikuanDialog();
			break;
		case R.id.tv_suershouh:
			showshouhuoDialog(makeId);
			break;
		case R.id.tv_sure_ocd:
			if (filePath1.size() == 1) {
				Toast.makeText(Activity_Pro_Custom.this, "请选择上传图片", Toast.LENGTH_SHORT).show();
				return;
			}
			...
			new Thread() {
				public void run() {
					for (int i = 1; i < filePath1.size(); i++) {
						...
						Bitmap resize_img = resize_img(BitmapFactory.decodeFile(filePath1.get(i)), yaso);
						...
						System.out.println("zuo===file==" + saveMyBitmap.getPath().toString() + "      size="
								+ (new File(filePath1.get(i))).length() + "   后=" + saveMyBitmap.length());
	...
	public void showshouhuoDialog(final int orderId) {
		...
		MyApplication.ahc.post(AppFinalUrl.usersureGetProduct, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				try {
					...
					if (result.equals("0")) {
						Toast.makeText(Activity_Pro_Custom.this, "收货成功", Toast.LENGTH_SHORT).show();
					}
	...
	private void showuikuanDialog() {
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setTitle("提示").setMessage("确定申请退款?").setPositiveButton("确定", new DialogInterface.OnClickListener() {
		...
		MyApplication.ahc.post(AppFinalUrl.userbackMoney, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				try {
					String result = response.getString("result");
					if (result.equals("0")) {
						Toast.makeText(Activity_Pro_Custom.this, "申请成功", Toast.LENGTH_SHORT).show();
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
		...
		builder.setView(view).setTitle("发起生产订单!").setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (ed_danjia.getText().length() == 0) {
					Toast.makeText(Activity_Pro_Custom.this, "请输入单价", Toast.LENGTH_SHORT).show();
				} else {
					dialog.dismiss();
		...
		MyApplication.ahc.post(AppFinalUrl.usercreateOrderToBuyer, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				try {
					String result = response.getString("result");
					if (result.equals("0")) {
						Toast.makeText(Activity_Pro_Custom.this, "发起成功", Toast.LENGTH_SHORT).show();
						dialog.dismiss();
					} else {
						Toast.makeText(Activity_Pro_Custom.this, "发起失败", Toast.LENGTH_SHORT).show();
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	public void initView() {
		...
		ll_order_pro_custom = (LinearLayout) findViewById(R.id.ll_order_pro_custom);
		tv_ordercode = (TextView) findViewById(R.id.tv_ordercode);
		tv_fist = (TextView) findViewById(R.id.tv_fist);
		// tv_suershouh, tv_applytuik
		tv_suershouh = (TextView) findViewById(R.id.tv_suershouh);
		tv_applytuik = (TextView) findViewById(R.id.tv_applytuik);
		tv_all = (TextView) findViewById(R.id.tv_all);
		tv_all.setText("合计:");
		tv_fist.setText("设计稿");
		tv_ordercode.setVisibility(View.VISIBLE);
		ll_order_pro_custom.setVisibility(View.VISIBLE);
		...
		MyApplication.ahc.post(AppFinalUrl.usergetMakeOrderInfoByType2, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				if (status == 0) {
					...
					tv_suershouh.setVisibility(View.GONE);
					tv_applytuik.setVisibility(View.GONE);
				}
				if (status == 1) {
	...
	public void Control() {
		...
		tv_faqi.setOnClickListener(this);// tv_suershouh, tv_applytuik
		tv_suershouh.setOnClickListener(this);
		tv_applytuik.setOnClickListener(this);
	}
	...
	class MyGVOnlineAdapter extends BaseAdapter {
		...
		public View getView(final int position, View v, ViewGroup parent) {
			v = View.inflate(Activity_Pro_Custom.this, R.layout.item_image_post, null);
			...
			im_commends.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					Intent intent = new Intent(Activity_Pro_Custom.this, Activity_LookPic.class);
	...
	class MyGVAdapter1 extends BaseAdapter {
		...
		@Override
		public View getView(final int position, View v, ViewGroup parent) {
			v = View.inflate(Activity_Pro_Custom.this, R.layout.item_gv_pic, null);
			...
			im.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					if (position == 0) {
						...
						intent.setClass(Activity_Pro_Custom.this, ImgFileListActivity.class);
						startActivity(intent);
					}
