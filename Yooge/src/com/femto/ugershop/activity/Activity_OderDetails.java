...
public class Activity_OderDetails extends BaseActivity {
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 1:
				...
				listThree.clear();
				listTwo.clear();
				Toast.makeText(Activity_OderDetails.this, "提交成功", Toast.LENGTH_SHORT).show();
				getData();
				break;
	...
	public void onClick(View v) {
		...
		case R.id.tv_sure_ocd:
			if (filePath1.size() == 1) {
				Toast.makeText(Activity_OderDetails.this, "请选择上传图片", Toast.LENGTH_SHORT).show();
				return;
			}
	...
	private void showFaqidialog() {
		...
		final EditText ed_count_oder = (EditText) view.findViewById(R.id.ed_count_oder);
		...
		builder.setView(view).setTitle("发起生产订单!").setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (ed_danjia.getText().length() == 0 || ed_count_oder.getText().length() == 0) {
					Toast.makeText(Activity_OderDetails.this, "请输入单价及数量", Toast.LENGTH_SHORT).show();
				} else {
					dialog.dismiss();
					faqidingdan(Integer.parseInt(ed_danjia.getText().toString()),
							Integer.parseInt(ed_count_oder.getText().toString()));
				}
	...
	private void faqidingdan(int price, int count) {
		...
		MyApplication.ahc.post(AppFinalUrl.usercreateOrderToBuyer, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				try {
					...
					if (result.equals("0")) {
						Toast.makeText(Activity_OderDetails.this, "发起成功", Toast.LENGTH_SHORT).show();
						dialog.dismiss();
					} else {
						Toast.makeText(Activity_OderDetails.this, "发起失败", Toast.LENGTH_SHORT).show();
					}
	...
	private void getData() {
		...
		MyApplication.ahc.post(AppFinalUrl.usergetMakeOrderInfoList, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				tv_math_ocd.setText("" + price * count + "*" + percentage * 100 + "%=" + price * count * percentage + "元");
				if (postCode == null || postCode.length() == 0 || postCode.equals("null")) {
					tv_ordercode_ocd.setHint("");
				...
				if (status == 3) {
					...
					tv_faqi.setVisibility(View.VISIBLE);
					tv_sure_ocd.setVisibility(View.GONE);
				}
				if (status == 4) {
	...
	private void setListOne() {
		...
		System.out.println("zuo===" + listOne.size() + "  " + listTwo.size() + "   " + listThree.size());
		if (listOne.size() != 0) {
			...
			System.out.println("zuo=dddddd==" + status);
			if (status == 5 || status == 13 || status == 14 || status == 15 || status == 6 || status == 0) {
				System.out.println("zuo=dfsfafdsafa==" + listOne.size() + "  " + listTwo.size() + "   " + listThree.size());
				ll_gv1.setVisibility(View.GONE);
	...
	private void setListTwo() {
		...
		if (listTwo.size() != 0) {
			...
			if (status == 5 || status == 13 || status == 14 || status == 15 || status == 6) {
				tv_sure_ocd.setVisibility(View.GONE);
			}
			System.out.println("zuosize==!0status==" + status);
		} else {
			System.out.println("zuosize==0status==" + status);
			if (status == 7) {
				ll_gv2.setVisibility(View.VISIBLE);
	...
	class MyGVOnlineAdapter extends BaseAdapter {
		...
		public View getView(final int position, View v, ViewGroup parent) {
			v = View.inflate(Activity_OderDetails.this, R.layout.item_image_post, null);
			...
			im_commends.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					Intent intent = new Intent(Activity_OderDetails.this, Activity_LookPic.class);
	...
	class MyGVAdapter1 extends BaseAdapter {
		...
		@Override
		public View getView(final int position, View v, ViewGroup parent) {
			v = View.inflate(Activity_OderDetails.this, R.layout.item_gv_pic, null);
			...
			im.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					if (position == 0) {
						...
						intent.setClass(Activity_OderDetails.this, ImgFileListActivity.class);
						startActivity(intent);
					}
