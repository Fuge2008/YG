...
import com.example.AndroidCaptureCropTags.util.CaoNiMeiToast;
...
public class TagsView extends FrameLayout implements TagView.TagViewListener {
	...
	public void setImage(String picpath, Context context) {
		Glide.with(context).load(picpath)
		/*
		 * 缺省的占位图片，一般可以设置成一个加载中的进度GIF图
		 */
		.crossFade().placeholder(R.drawable.tianc).into(backImage);
		// /ImageLoader.getInstance().displayImage(picpath, backImage,
		// MyApplication.getOptions());
		// backImage.setImageBitmap(BitmapUtil.loadBitmap("/storage/emulated/0/DCIM/Camera/IMG_20151020_205156_HDR.jpg"));
	}
